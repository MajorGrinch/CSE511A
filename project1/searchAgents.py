...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not hitsWall:
                vis_cor = list(state[1])
                if next_node in self.corners and next_node not in vis_cor:
                    vis_cor.append(next_node)
                successor = ((next_node, vis_cor), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    position = state[0]
    vis_cors = list(state[1])
    if problem.isGoalState(state):
        return 0
     
    unvis_cors = list(corners)
    for cor in vis_cors:
        unvis_cors.remove(cor)

    current_pos = position
    h = 0
    while len(unvis_cors) != 0:
        idx, dist = findClosestCorners(current_pos, unvis_cors)
        h += dist
        current_pos = unvis_cors[idx]
        del unvis_cors[idx]
        # unvis_cors.remove(unvis_cors[idx])

    return h


def findClosestCorners(cur_po, corners):
    idx = -1
    min_dist = float('inf')
    for i in range(len(corners)):
        dist = util.manhattanDistance(cur_po, corners[i])
        if min_dist > dist:
            min_dist = dist
            idx = i
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        maxDistance = max(maxDistance,mazeDistance(position, food,problem.startingGameState))
